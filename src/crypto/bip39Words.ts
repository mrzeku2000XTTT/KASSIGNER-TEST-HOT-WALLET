...
// Standard BIP-0039 English wordlist (2048 words)
export const BIP39_WORDLIST: string[] = [
  ...
  "only","onto","open","opinion","oppose","option","orange","orbit","orchard","order",
  ...
  "pin","pine","pipe","pistol","pitch","pizza","place","planet","plastic","plate",
  ...
  "spy","squad","square","squash","squirrel","stable","stadium","staff","stage","stairs",
  ...
  "useless","usual","utility","vacant","vacuum","vague","valuable","van","vanish","vapor",
  ...
  "week","weight","welcome","west","wet","whale","what","wheat","wheel","when",
...
export function isValidWord(word: string): boolean {
  return wordMap.has(word.toLowerCase().trim());
}

export function generateEntropy(bytes = 16): Uint8Array {
...
export function validateMnemonic(mnemonic: string): { valid: boolean; error?: string } {
  ...
  if (words.length !== 12 && words.length !== 24) {
    return { valid: false, error: `Seed must be 12 or 24 words (got ${words.length})` };
  }

  for (let i = 0; i < words.length; i++) {
    if (!wordMap.has(words[i])) {
      return { valid: false, error: `Invalid BIP39 word at position #${i + 1}: "${words[i]}"` };
    }
  }

  // Verify checksum
  const bits = words.map(w => {
    const idx = wordMap.get(w)!;
    return idx.toString(2).padStart(11, '0');
  }).join('');

  const entropyBitsLen = words.length === 12 ? 128 : 256;
  const checksumBitsLen = words.length === 12 ? 4 : 8;
  const entropyBits = bits.slice(0, entropyBitsLen);
  const checksumBits = bits.slice(entropyBitsLen, entropyBitsLen + checksumBitsLen);

  const entropyBytes = bitsToBytes(entropyBits);
  const hash = sha256(entropyBytes);
  const expectedChecksumBits = bytesToBits(hash).slice(0, checksumBitsLen);

  if (checksumBits !== expectedChecksumBits) {
    return { valid: false, error: 'Mnemonic checksum verification failed. Please check the words.' };
  }
...
/**
 * Dice Roll entropy calculator (5 rolls per word or raw 0-5 base-6)
 */
export function diceRollsToEntropy(rolls: number[]): Uint8Array {
  // rolls are 1-6
  const hex = sha256(new Uint8Array(rolls));
